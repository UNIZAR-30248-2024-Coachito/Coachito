...
export const mapWorkoutExerciseDBToDetails = (
  workoutExercises: WorkoutExerciseDB[]
): ExerciseResume[] => {
  const exercisesMap = workoutExercises
    .sort(
      (a, b) =>
    ...
    .reduce<Record<string, ExerciseResume>>((acc, exercise) => {
      ...
      const newSeries: SeriesExerciseResume[] = Array.from({
        ...
        reps: exercise.reps,
        targetReps: exercise.target_number_reps ?? 0,
